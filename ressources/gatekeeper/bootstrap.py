...
def get_gatekeeper_user_data(
    ...
    gatekeeper_user_data = GATEKEEPER_USER_DATA
    
    main_script =  escape_single_quotes(var_env_temp + "\n" + open("ressources/gatekeeper/gatekeeper.py", "r").read())
    main_script_creation = f"""
        echo '{main_script}' > /home/ubuntu/my_project/gatekeeper.py
    """
    gatekeeper_user_data += "\n" + main_script_creation + "\n" + LAUNCH_SERVER_COMMAND
    return gatekeeper_user_data
